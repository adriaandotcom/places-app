...
@Test func wifiSuggestionMigrationBackfillsPreservesRawHistoryAndErasesIndex() async throws {
    ...
    let beforeData = try await store.exportHistory()
    var before = try #require(JSONSerialization.jsonObject(with: beforeData) as? [String: Any])
    before.removeValue(forKey: "exportedAt")
    ...
    let afterData = try await migrated.exportHistory()
    var after = try #require(JSONSerialization.jsonObject(with: afterData) as? [String: Any])
    after.removeValue(forKey: "exportedAt")
    #expect((before as NSDictionary).isEqual(to: after))
